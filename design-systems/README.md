...
**使う順番：** Claude Design は「① デザインシステム → ② ワイヤーフレーム → ③ デザイン」の順で作ると崩れません。このデザインシステムは、その **①（最初の土台）** にあたります。**Claude のデスクトップアプリ**の Claude Design でこのフォルダ（または各 `tokens.json`）を読み込ませ、土台にしてからワイヤーフレーム→デザインへ進んでください。
